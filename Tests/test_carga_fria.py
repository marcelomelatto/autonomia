...
def test_id_tipo_local_sem_nulos():
    df = carrega_df_dim_local_atlas()
    assert (
        df["id_local"].notnull().all()
    ), "t002 = [df_dim_local_atlas] = coluna 'id_local' contém valores nulos"
    assert (
        df["cod_local_sap"].notnull().all()
...
def test_id_local_sem_duplicatas():
    df = carrega_df_dim_local_atlas()
    assert df[
        "id_local"
    ].is_unique, (
...
def test_tipo_local_valores_validos():
    df = carrega_df_dim_local_atlas()
    valid_status = {"AGENTE", "BASE", "EPO", "LOJA", "TECNICO"}
    assert set(df["tipo_local"].dropna().unique()).issubset(
        valid_status
...
def test_centros_abastecimento_validos():
    df = carrega_df_dim_local_atlas()
    ...
    assert (
        df["cod_centro_abastecimento_1"].astype(str).str.match(centro_pattern).all()
    ), "t005 = [df_dim_local_atlas] = 'cod_centro_abastecimento_1' contém valores inválidos"
    assert (
        df["cod_centro_abastecimento_2"].astype(str).str.match(centro_pattern).all()
...
def test_empresa_valores_validos():
    df = carrega_df_dim_local_atlas()
    valid_status = {"CLARO TV", "NET"}
    assert set(df["empresa"].dropna().unique()).issubset(
        valid_status
...
def test_status_abastecimento_valores_validos():
    df = carrega_df_dim_local_atlas()
    valid_status = {"INATIVO", "ATIVO"}
    assert set(df["status_abastecimento"].dropna().unique()).issubset(
        valid_status
...
def test_status_familia_sem_nulos():
    df = carrega_df_dim_familia()
    assert (
        df["status_familia"].notnull().all()
...
def test_dsc_familia_sem_duplicatas():
    df = carrega_df_dim_familia()
    assert df[
        "dsc_familia"
    ].is_unique, (
...
def test_status_familia_valores_validos():
    df = carrega_df_dim_familia()
    valid_status = {"ATIVO", "INATIVO"}
    assert set(df["status_familia"].dropna().unique()).issubset(
        valid_status
...
def test_pelo_menos_um_ativo():
    df = carrega_df_dim_familia()
    assert (
        df["status_familia"] == "ATIVO"
    ).any(), "t005 = [df_dim_familia] = nenhuma familia com  status 'ativo'"


# df_dim_familia
# t006. Testar se não há valores nulos em "dsc_familia"
def test_dsc_familia_sem_nulos():
    df = carrega_df_dim_familia()
    assert (
        df["dsc_familia"].notnull().all()
...
def test_dataframe_min_linhas():
    df = carrega_df_dim_familia()
    assert len(df) >= 90, "t007 = [df_dim_familia] = dataframe com menos de 90 linhas"
